    ...
    useEffect(() => {
        async function fetchPolls() {
            try {
                setLoading(true);
                setError("");
                const res = await fetch(`/api/poll/get_user_polls?user_id=${user_id}`)

                const data = await res.json();
                if (res.ok) {
                    setPolls(data.poll_data || [])
                } else {
        ...
        return (
            <section className='mt-10 w-full'>
                <div className='flex justify-center'>
                    <div className='animate-pulse text-gray-400'>Loading your polls...</div>
                </div>
            </section>
        );
    }
        ...
        return (
            <section className='mt-10 w-full'>
                <div className='border-2 bg-[#f3f4f6] p-8 text-center'>
                    ...
                    <p className='text-gray-600 font-semibold mb-2'>No Polls Yet</p>
    ...
    return (
        <section className='mt-8 w-full lg:w-1/2'>
            <div className='mb-4 px-2 sm:px-0'>
                <h2 className='text-xl sm:text-2xl font-bold text-black'>Your Polls</h2>
                <p className='text-gray-500 text-xs sm:text-sm mt-1'>{polls.length} poll{polls.length !== 1 ? 's' : ''} created</p>
            </div>

            <div className='max-h-[400px] sm:max-h-[600px] overflow-y-auto pr-2 space-y-2 sm:space-y-3'>
                {polls.map((poll: Poll_data) => {
